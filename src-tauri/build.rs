...
//! Requires Intel oneAPI Base Toolkit (MKL component) installed at the
//! default path: C:\Program Files (x86)\Intel\oneAPI\mkl\latest\
...
fn main() {
    ...
    // --- Intel MKL paths ---
    let mkl_root = r"C:\Program Files (x86)\Intel\oneAPI\mkl\latest";
    let mkl_include = format!(r"{}\include", mkl_root);
    let mkl_lib = format!(r"{}\lib", mkl_root);

    // Verify MKL is installed
    if !std::path::Path::new(&mkl_include).join("mkl_dfti.h").exists() {
        panic!(
            "Intel MKL not found at {}. \
             Install Intel oneAPI Base Toolkit (MKL component) from: \
    ...
    // --- Link Windows system libraries needed by npcap capture layer ---
    #[cfg(target_os = "windows")]
    {
